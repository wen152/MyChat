...
{
	LOG_INFO << "LogicSystem::PostMsgToQue";
	if (!msg) {
        LOG_ERROR << "Received nullptr LogicNode in PostMsgToQue";
        return;
    }
...
void LogicSystem::DealMsg()
{
	for (;;)
	{
		...
		LOG_INFO << "msg id is " << msg_node->_recvnode->_msg_id;
		LOG_INFO << "msg data is " << msg_node->_recvnode->_data;
		LOG_INFO << "msg len is " << msg_node->_recvnode->_cur_len;//
...
void LogicSystem::LoginHandler(shared_ptr<CSession> session, const short &msg_id, const string &msg_data)
{
	...
	Defer defer([this, &rtvalue, session]()
		...
		LOG_INFO << "return_str is " << return_str;
		session->Send(return_str, MSG_CHAT_LOGIN_RSP); });

	// 从redis获取用户token是否正确
	std::string uid_str = std::to_string(uid);
	std::string token_key = USERTOKENPREFIX + uid_str; // utoken_1059
	std::string token_value = "";
	bool success = RedisMgr::GetInstance()->Get(token_key, token_value);
	if (!success)
	{
		rtvalue["erron"] = ErrorCodes::UidInvalid;
		return;
	}

	if (token_value != token)
	{
		rtvalue["erron"] = ErrorCodes::TokenInvalid;
		return;
	}
...
void LogicSystem::AddFriendApply(std::shared_ptr<CSession> session, const short &msg_id, const string &msg_data)
{
	...
	if (to_ip_value == self_name)
	{
		...
		if (session)
		{
			// 在内存中则直接发送通知对方
			Json::Value notify;
			notify["erron"] = ErrorCodes::Success;
			notify["applyuid"] = uid;
...
void LogicSystem::AuthFriendApply(std::shared_ptr<CSession> session, const short &msg_id, const string &msg_data)
{
	...
	if (to_ip_value == self_name)
	{
		...
		if (session)
		{
			...
			{
				notify["erron"] = ErrorCodes::UidInvalid;
			}
...
bool LogicSystem::GetBaseInfo(std::string base_key, int uid, std::shared_ptr<UserInfo> &userinfo)
{
	...
	else
	{
		...
		RedisMgr::GetInstance()->Set(base_key, redis_root.toStyledString()); // 登录的数据写入redis缓存 方便后面查询
	}
